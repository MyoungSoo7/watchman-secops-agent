...
| ASI10 | Rogue Agents | 쓰기 경로 없음, 이그레스 허용목록, 감사로그 | — | 🟡 부분 — 감사로그 변조 *탐지*(해시 체인 + ELK 닻), 방지는 아님 |
...
- 감사로그는 에이전트가 쓰는 PVC 의 JSONL 이다. 2026-09-25 부터 줄마다 직전 줄의 sha256(`prev`)을 실어 **중간 줄 변조·삭제는 기동 시 `verify_audit_chain` 이 잡는다.** 다만 파드를 탈취한 공격자는 체인 전체를 새로 계산해 다시 쓸 수 있다 — 이건 stdout(→ELK) 에 찍힌 줄 해시 `h=` 와 대조해야 드러나고, 그 대조는 자동화돼 있지 않다.
...
- 탐지층 자체의 침묵을 감시한다(2026-09-25). 노드별 Falco 커널 이벤트율이 0 이면 `FalcoSilent` 가 울리고, 노드에 up 인 Falco 가 없으면 `FalcoNodeUncovered` 가 울린다. 둘 다 15분 뒤 critical 로 watchman 을 거치지 않고 텔레그램에 직행한다. 평시 발화는 0건이고, 조건을 반전하면 6노드 모두가 잡히는 것을 확인했다. Falco 를 실제로 꺼 보는 음성 실측은 하지 않았다.
...
- 2026-09-25 이그레스를 좁혔다. K8s API 는 서비스망 /16 → API ClusterIP /32, 노드망 /24 → 컨트롤플레인 3대 /32 의 6443 이다. 인바리언트 I4 의 RDP 점검도 노드 6대 /32 로 좁혔다. 전후 15개 목적지를 실측했다. 필요한 7개 경로는 계속 열려 있고 차단 대상 8개는 계속 거부된다. 정직하게 적으면, 넓었던 부분은 원래도 닫힌 포트였다. 이번 변경은 실제로 막힌 경로가 새로 생긴 것이 아니라 허용 범위를 최소로 줄인 것이다.
- 네임스페이스에 PSA `warn/audit=restricted` 를 걸었다. `enforce=restricted` dry-run 에서 watchman 파드는 통과했다. 같은 네임스페이스의 다른 파드 3종이 위반이라 enforce 는 걸지 않았다.
...
- 감사로그는 에이전트 자신이 쓰는 PVC 라 변조 *방지*는 없다. 해시 체인이 중간 변조를 탐지하고, 체인 재작성·꼬리 절단은 ELK 의 `h=` 닻과 수동 대조로만 드러난다(ASI06 참고).
